                             ...
                             NodePosMap &posy)
{
  ...
  {
    linha = "\t";
    linha += IntToString(g.id(g.u(a)));
    linha += "  -- ";
    linha += IntToString(g.id(g.v(a)));
    linha += ";\n";
    out << linha;
  }
  out << "}\n";
                             ...
                             DNodePosMap &posy)
{
  ...
  {
    linha = "\t";
    linha += IntToString(g.id(g.source(a)));
    linha += "  -> ";
    linha += IntToString(g.id(g.target(a)));
    linha += ";\n";
    out << linha;
  }
  out << "}\n";
...
bool GraphTable::GetNodeCoordinates(string colx, NodePosMap &posx, string coly, NodePosMap &posy)
{
  // read from file
  if ((this->NodeTable->columnindex(colx) != -1) && (this->NodeTable->columnindex(coly) != -1))
  {
    this->GetColumn(colx, posx);
    this->GetColumn(coly, posy);
  }
  // no positions in the file, call a routine to create positions for the nodes
  else
    GenerateVertexPositions(this->g, posx, posy);
               ...
               EdgeValueMap &weight)
{
  ...
  ok = ok && GT.GetNodeCoordinates(mygraphlibparameters.inputgraphtable_posx, posx,
                                    ...
                                    double SizeY)          // coordinate y is a random number in [0,SizeY)
{
  ...
  for (int i = 0; i < n; i++)
  {                     // insert nodes (random points in [0,100] x [0,100] )
                               ...
                               double SizeY)         // coordinate y is a random number in [0,SizeY)
{
  ...
  if (r8tris2(n, p, &ntri, tri, tri_nabe))
  {
    printf("ERROR\n");
    Pause();
  }
                                 ...
                                 double SizeY)          // coordinate y is a random number in [0,SizeY)
{
  ...
  for (i = 0; i < n; i++)
  {                               // insere os vértices (pontos aleatórios no plano [0,SizeX]x[0,SizeY] )
    V[i] = g.addNode();           // gera um vértice nó do grafo
    px[V[i]] = SizeX * drand48(); // nodes are random points
    py[V[i]] = SizeY * drand48();
    p[2 * i] = px[V[i]];
...
GraphAttributes::GraphAttributes(Graph &graph, NodeStringMap &vname,
                                 NodePosMap &posx, NodePosMap &posy) : g(graph), node_name(graph), px(graph), py(graph), node_attrib(graph), edge_attrib(graph)
{
  factor = 1.0;
  graph_attrib = "";
...
GraphAttributes::GraphAttributes(GraphTable &GT) : g(GT.g), node_name(GT.g), px(GT.g), py(GT.g), node_attrib(GT.g), edge_attrib(GT.g)
{
  factor = 1.0;
  graph_attrib = "";
  GT.GetColumn(mygraphlibparameters.inputgraphtable_nodename, node_name);
  GT.GetNodeCoordinates(mygraphlibparameters.inputgraphtable_posx, px,
  ...
  GT.GetNodeCoordinates(mygraphlibparameters.inputgraphtable_posx, this->px,
...
bool GraphAttributes::View() // text displayed below the figure
{
  ...
  for (EdgeIt e(this->g); e != INVALID; ++e)
  {
    if (IsSuffix(this->edge_attrib[e], "color=\"Invis\""))
      continue; // only to speed graphviz exec.
...
// graphviz.org) to generate a pdf file and a program to view the pdf file. The
// pdf viewer name is given in the viewername parameter.
bool GraphAttributes::Save(string name) // text displayed below the figure
{
  char tempname[1000], cmd[1000], outputname[1000];
  FILE *fp;
  string str;
...
DigraphAttributes::DigraphAttributes(Digraph &digraph, DNodeStringMap &vname,
                                     DNodePosMap &posx, DNodePosMap &posy) : g(digraph), node_name(vname), px(posx), py(posy), node_attrib(digraph), arc_attrib(digraph)
{
  factor = 1.0;
  digraph_attrib = "";
...
AdjacencyMatrix::AdjacencyMatrix(Graph &graph, EdgeValueMap &graphweight, double NonEdgValue) : Node2Index(graph), Edge2Index(graph)
{
  int i;
  g = &graph;
  ...
  for (EdgeIt e(graph); e != INVALID; ++e)
  {
    ...
    i_v = Node2Index[v];
    if (i_u > i_v)
      AdjMatrix[i_u * (i_u - 1) / 2 + i_v] = graphweight[e];
    else if (i_u < i_v)
...
double AdjacencyMatrix::Cost(Node u, Node v)
{
  ...
  {
    if (i_u > i_v)
      return (AdjMatrix[i_u * (i_u - 1) / 2 + i_v]);
    else if (i_u < i_v)
...
bool GreedyColoring(Graph &g, int &ncolors, NodeIntMap &nodecolor)
{
  ...
  for (int i = 0; i < n; i++)
  { // in each loop, we get one node of smallest degree
    Node v;
    int dv;
  ...
  vector<int> adjacentcolors(maxremoveddegree);
  for (NodeIt v(g); v != INVALID; ++v)
    nodecolor[v] = -1; // means no color
  for (int i = 0; i < n; i++)
  {
    ...
    sort(adjacentcolors.begin(), adjacentcolors.begin() + nadjcolors);
...

    {
      ...
      { // has at least two nodes and the first color is 0
        for (j = 0; j < nadjcolors - 1; j++)
          if (adjacentcolors[j + 1] - adjacentcolors[j] > 1)
            break;
