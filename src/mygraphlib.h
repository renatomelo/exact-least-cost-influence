...
{
  for (EdgeIt a(this->g); a != INVALID; ++a)
    this->SetLabel(a, x[a]);
}
...
{
  for (NodeIt v(this->g); v != INVALID; ++v)
    if ((nodeval[v] > lb - MY_EPS) && (nodeval[v] < ub + MY_EPS))
      this->SetColor(v, cor);
}
...
{
  for (EdgeIt e(this->g); e != INVALID; ++e)
    if ((edgeval[e] > lb - MY_EPS) && (edgeval[e] < ub + MY_EPS))
      this->SetColor(e, cor);
}
...
{
  for (ArcIt e(this->g); e != INVALID; ++e)
    if ((arcval[e] > lb - MY_EPS) && (arcval[e] < ub + MY_EPS))
      this->SetColor(e, cor);
}
// ----
